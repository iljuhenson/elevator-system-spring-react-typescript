        ...
        <>
            {Array.from(Array(30).keys()).reverse().map(floorIdx => {
                return (<div key={floorIdx} className="floor-control">
                    <div onClick={() => typeof pickupFloor === "number" ? addFloor(pickupFloor!, direction!, floorIdx) : false} className={`floor-label ${typeof pickupFloor === "number" ? "floor-label-active" : "floor-label-inactive"}`}><p className="floor-label-text">{floorIdx}</p></div>
